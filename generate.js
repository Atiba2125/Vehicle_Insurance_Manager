...
models.forEach(m => {
    const dir = path.join(basePath, m.Controller);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });

    const indexContent = `@model IEnumerable<VehicleShield.Models.${m.ModelName}>
...
<div class="auth-layout" style="min-height: calc(100vh - 80px); padding: 40px 20px; align-items:flex-start;">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover">
                <thead style="background: linear-gradient(135deg, #6C63FF 0%, #3ECFCF 100%); color: white;">
                    <tr><th>ID</th><th>Details</th><th>Actions</th></tr>
                </thead>
                <tbody>
    ...
    fs.writeFileSync(path.join(dir, "Index.cshtml"), indexContent);

    const createContent = `@model VehicleShield.Models.${m.ModelName}
...
<div class="auth-layout" style="min-height: calc(100vh - 80px); padding: 40px 20px;">
    <div class="auth-card" style="max-width: 600px; width: 100%;">
        ...
        <form asp-action="Create" method="post" class="auth-form">
            ...
            <div class="form-group"><button type="submit" class="btn-auth">Create</button></div>
        </form>
        <a asp-action="Index" style="color:#6C63FF; text-decoration:none;">Back to List</a>
    </div>
</div>`;
    fs.writeFileSync(path.join(dir, "Create.cshtml"), createContent);

    const editContent = `@model VehicleShield.Models.${m.ModelName}
...
<div class="auth-layout" style="min-height: calc(100vh - 80px); padding: 40px 20px;">
    <div class="auth-card" style="max-width: 600px; width: 100%;">
        ...
        <form asp-action="Edit" method="post" class="auth-form">
            ...
            <div class="form-group"><button type="submit" class="btn-auth">Save</button></div>
        </form>
        <a asp-action="Index" style="color:#6C63FF; text-decoration:none;">Back to List</a>
    </div>
</div>`;
    fs.writeFileSync(path.join(dir, "Edit.cshtml"), editContent);

    const detailsContent = `@model VehicleShield.Models.${m.ModelName}
@{ ViewData["Title"] = "Details ${m.Name}"; }
<div class="auth-layout" style="min-height: calc(100vh - 80px); padding: 40px 20px;">
    <div class="auth-card" style="max-width: 800px; width: 100%;">
        <h2>${m.Name} Details</h2>
        <div><hr /><dl class="row"><dt class="col-sm-3">Information</dt><dd class="col-sm-9">@Model.ToString()</dd></dl></div>
    ...
    fs.writeFileSync(path.join(dir, "Details.cshtml"), detailsContent);

    const deleteContent = `@model VehicleShield.Models.${m.ModelName}
@{ ViewData["Title"] = "Delete ${m.Name}"; }
<div class="auth-layout" style="min-height: calc(100vh - 80px); padding: 40px 20px;">
    <div class="auth-card" style="max-width: 800px; width: 100%;">
        <h2>Delete ${m.Name}</h2>
        <h3 style="color:#ff6b6b; margin-bottom:20px;">Are you sure you want to delete this?</h3>
        <div><hr /><dl class="row"><dt class="col-sm-3">Information</dt><dd class="col-sm-9">@Model.ToString()</dd></dl>
    ...
    fs.writeFileSync(path.join(dir, "Delete.cshtml"), deleteContent);
});

const reportDir = path.join(basePath, "Reports");
if (!fs.existsSync(reportDir)) fs.mkdirSync(reportDir, { recursive: true });
const reportContent = `@{ ViewData["Title"] = "Reports"; }
<div class="auth-layout" style="min-height: calc(100vh - 80px); padding: 40px 20px; align-items:flex-start;">
    <div class="auth-card" style="max-width: 1200px; width: 100%;">
...
fs.writeFileSync(path.join(reportDir, "Index.cshtml"), reportContent);
